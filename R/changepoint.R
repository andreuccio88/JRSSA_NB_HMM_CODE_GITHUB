# Hierarchical single-change-point model helpers.

build_changepoint_data <- function(dat, cfg, cohorts_global = NULL) {
  ...
  if (A < 4L || P < 5L || C_global < 5L) stop("Lexis grid too small for change-point.")
  ...
  if (anyNA(dat_indexed$country_id) || anyNA(dat_indexed$age_idx) ||
    ...
    stop("Internal change-point indexing failure.", call. = FALSE)
  }
  ...
  for (i in seq_len(I)) {
    ...
    if (any(diff(obs_c) != 1L)) {
      stop("change-point requires contiguous supported cohort grids within country: ",
...
fit_changepoint_model <- function(pack, cfg, stan_file, output_dir,
  ...
  fit <- model$sample(
    ...
    iter_warmup = as.integer(iter_warmup %||% cfg$sampling$changepoint_primary$iter_warmup),
    iter_sampling = as.integer(iter_sampling %||% cfg$sampling$changepoint_primary$iter_sampling),
    adapt_delta = as.numeric(adapt_delta %||% cfg$sampling$changepoint_primary$adapt_delta),
    max_treedepth = as.integer(max_treedepth %||% cfg$sampling$changepoint_primary$max_treedepth),
    refresh = cfg$sampling$refresh,
    init = init,
    output_basename = "changepoint"
  )
  if (any(fit$return_codes() != 0L)) stop("One or more change-point Stan chains failed.", call. = FALSE)
  fit$save_output_files(dir = csv_dir, basename = "changepoint", timestamp = FALSE, random = FALSE)
  fit$save_object(file.path(output_dir, "fit_cmdstanr.rds"))
  fit
}

summarise_changepoint_fit <- function(fit, pack, output_dir) {
