# Smooth-cohort comparator helpers.
...
fit_smooth_cohort_model <- function(pack,cfg) {
  model <- compile_project_stan_model(cfg$stan_smooth_file, cfg, force_recompile = FALSE)
  final_csv <- file.path(cfg$output_dir,'fit','csv'); clean_directory(final_csv)
  tune <- cfg$sampling$smooth
  message('Smooth-cohort comparator: ', cfg$sampling$chains, ' chains; ', tune$iter_warmup, ' warmup + ', tune$iter_sampling, ' sampling.')
  fit <- model$sample(
    ...
    iter_warmup=as.integer(tune$iter_warmup), iter_sampling=as.integer(tune$iter_sampling),
  ...
  if(any(fit$return_codes()!=0L)) stop('One or more smooth-cohort Stan chains failed.', call. = FALSE)
