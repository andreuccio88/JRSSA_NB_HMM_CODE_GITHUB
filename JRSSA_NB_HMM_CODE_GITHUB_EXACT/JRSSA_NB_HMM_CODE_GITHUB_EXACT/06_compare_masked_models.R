...
cp_file <- file.path(cfg$paths$real_full,'V7_Light_CP','masked_target_1925_1935','tables','masked_predictive_density_by_country.csv')
sm_file <- file.path(cfg$paths$real_full,'Smooth','masked_target_1925_1935','tables','masked_predictive_density_by_country.csv')
...
outdir <- file.path(cfg$paths$real_full,'comparison'); dir.create(outdir, recursive=TRUE, showWarnings=FALSE)
readr::write_csv(by_country, file.path(outdir,'masked_target_comparison_by_country.csv'))
# Number of held-out country-age-period cells (sex likelihoods are jointly scored per cell).
pack <- readRDS(file.path(cfg$paths$real_full,'V7_Light_CP','masked_target_1925_1935','stan_pack.rds'))
n_masked <- sum(pack$stan_data$include_likelihood == 0L)
summary <- data.frame(
  target_start=cfg$analysis_spec$target_mask_start,
  target_end=cfg$analysis_spec$target_mask_end,
